...
from utils.normalizers import (
    ...
    extract_email, extract_phone_numbers, normalize_category,
    extract_urls, extract_social_handles
)
...
class JustdialScraper(BaseScraper):
    """Scraper for Justdial.com listings."""

    def __init__(self, settings: Settings, database: Database):
        super().__init__(settings, database)
        self.base_url = settings.platforms["justdial"]["base_url"]
        self.platform = "justdial"

    async def discover_categories(self) -> List[str]:
        """Discover categories from Justdial."""
        logger.info("Discovering Justdial categories")
    ...
    async def discover_listings(self, category: str, locality: str) -> List[str]:
        ...
        try:
            search_query = f"{category} {locality}"
            ...
            if response:
                soup = BeautifulSoup(response.content, "html.parser")
                listing_divs = soup.find_all("div", {"data-id": True})
    ...
    def _extract_name(self, soup: BeautifulSoup) -> Optional[str]:
        """Extract provider name from soup."""
        name_elem = soup.find("h1", class_="fn")
        return name_elem.get_text(strip=True) if name_elem else None

    def _extract_phones(self, soup: BeautifulSoup) -> List[str]:
        """Extract phone numbers from listing."""
        phones = []
        phone_elem = soup.find("span", {"data-phone": True})
        if phone_elem:
    ...
    def _extract_email(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        email_elem = soup.find("a", {"data-email": True})
        return email_elem.get("data-email") if email_elem else None

    def _extract_address(self, soup: BeautifulSoup) -> Optional[str]:
        """Extract address from listing."""
        address_elem = soup.find("div", class_="vcard")
        if address_elem:
    ...
    def _extract_review_count(self, soup: BeautifulSoup) -> Optional[int]:
        ...
        review_elem = soup.find("span", class_="review-count")
        if review_elem:
            try:
                count = review_elem.get_text(strip=True)
    ...
    def _extract_images(self, soup: BeautifulSoup) -> Optional[List[str]]:
        """Extract image URLs."""
        images = []
        img_elements = soup.find_all("img", class_="photo")
